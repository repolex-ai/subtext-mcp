...
```bash
git clone https://github.com/repolex-ai/subtext-mcp.git ~/subtext-mcp   # or wherever you like
cd ~/subtext-mcp
bun install
```
...
This makes subtext available in every Claude Code session, from any directory:

```bash
claude mcp add --scope user --transport stdio subtext -- bun ~/subtext-mcp/server.ts
```

Replace `~/subtext-mcp` with wherever you cloned it.

### 3. Run Claude Code with the channel

```bash
claude --dangerously-skip-permissions --dangerously-load-development-channels server:subtext
```
...
>
> ```bash
> alias claudesubtext='claude --dangerously-load-development-channels server:subtext'
> ```
...
| -------------------- | ---------------- | ------------------------------------- |
| `SUBTEXT_PORT`       | `7899`           | Broker port                           |
| `SUBTEXT_DB`         | `~/.subtext.db`  | SQLite database path                  |
...
Based on [claude-peers](https://github.com/louislva/claude-peers-mcp) by louislva.
